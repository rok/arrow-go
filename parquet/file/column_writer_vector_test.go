...
// the vector length) is a programming error and must be rejected rather than
// producing a malformed column.
func TestVectorColumnWriterRejectsPartialVector(t *testing.T) {
	const vectorLen = 4
	descr := vectorColumnDescr(t, vectorLen)
	cw, _ := newVectorChunkWriter(t, descr, parquet.NewWriterProperties(parquet.WithDictionaryDefault(false)))

	// 10 is not a multiple of vectorLen=4.
