...
// pathological nested-vector schema that would overflow int32 panics rather
// than silently wrapping.
func effectiveVectorLength(n Node) int32 {
	eff := int64(-1)
	for cursor := n; cursor != nil; cursor = cursor.Parent() {
		...
		if eff > math.MaxInt32 {
			panic("parquet: nested VECTOR effective vector_length overflows int32")
		}
	}
	return int32(eff)
}
...
func NewColumn(n *PrimitiveNode, maxDefinitionLvl, maxRepetitionLvl int16) *Column {
	return &Column{
		...
		path:                  n.Path(),
		effectiveVectorLength: effectiveVectorLength(n),
	}
}
